...
for ($i = 1; $i <= $japCount; $i++) {
    // echo "SELECT * FROM packages WHERE `type`='followers' AND `socialmedia`='tt' GROUP BY `jap$i` ORDER BY id DESC" . "<br>";
    $Query = mysql_query("SELECT * FROM packages WHERE `type`='followers' AND `socialmedia`='tt' GROUP BY `jap$i` ORDER BY id DESC");
    // $img = 'followers-grid.png';
    $time = time();
    while ($row = mysql_fetch_array($Query)) {
        $bucket = 'https://etra-live-japqc.s3.us-east-2.amazonaws.com/media';
        $img = $bucket . '/tt_followers_' . $row['jap' .$i] . '.png';
        ...
        echo $score . "<br>";
        $update_eq = mysql_query("UPDATE `packages` SET `jap".$i."_qc`='" . $score . "', `qc` = '$time' WHERE `jap$i`=" . $row['jap' .$i] . " LIMIT 1");
    }
