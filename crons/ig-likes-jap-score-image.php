...
<?php if (!isset($_GET['frame'])): ?>
    <!-- Outer loader -->
    <h2>Likes Snapshot (Rendered in Iframe)</h2>
...
<?php else: ?>
    ...
    function next() {
        ...
        allImagesLoaded(section, () => {
            html2canvas(section, { useCORS: true }).then(canvas => {
                ...
                document.getElementById('filenameField').value = `likes_${jap}.png`;
    ...
    function getLikes($post)
    {
        ...
        $url = 'https://i.supernova-493.workers.dev/api/v3/post?short_code=' . $post;
        ...
        $postId = $users['items'][0]['id'];
        // echo "Post ID: $postId <br>";
        $url = 'https://i.supernova-493.workers.dev/api/v3/likes?post_id=' . $postId;
        curl_setopt($curl, CURLOPT_URL, $url);
        $get = curl_exec($curl);
        curl_close($curl);
        ...
        // print_r($get); die;
        foreach ($get['data']['users'] as $user) {
            if ($i >= 50) break;
            $data[] = [
                'userId' => $user['pk'],
    ...
    for ($i = 1; $i <= $japCount; $i++) {
        $q = "SELECT id, `jap$i` as jap FROM `packages` WHERE `type`='likes' AND `socialmedia`='ig'  GROUP BY `jap$i` ORDER BY id DESC";
        ...
        while ($dataP = mysql_fetch_array($result)) {
            ...
            while ($row = mysql_fetch_array($result2)) {
                ...
                $chooseposts = $row['chooseposts'];

                $posts = explode(' ', $chooseposts);
                $post = $posts[0];
                // print_r($posts);
                ...
                echo "<div class='title'>Likes of post @$post</div>";

                if (is_array($data) && count($data) > 0) {
                    echo "<div class='likes-grid'>";
                    foreach ($data as $follower) {
                        $username = htmlspecialchars($follower['userUsername']);
                        $fullname = htmlspecialchars($follower['userFullName']);
                    ...
                    echo "<div>No users found for @$post</div>";
                }
