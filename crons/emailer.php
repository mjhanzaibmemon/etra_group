...
try {
    ...
    sendCloudwatchData('EtraGroupCrons', 'email-success-abandoned', 'AbandonedCheckout', 'abandoned-checkout-success-function', 1);
  ...
  //  echo "<br><b style='color:red'>AN ERROR OCCURED</b><br> {$e->errorMessage()}", PHP_EOL; //Catch errors from PHPMailer.
    ...
    sendCloudwatchData('EtraGroupCrons', 'email-failure-abandoned', 'AbandonedCheckout', 'abandoned-checkout-failure-function', 1);
    writeCloudWatchLog('Crons-Emailer', $recipient .' Caught exception: '.  $e->errorMessage());
} catch (Exception $e) {
    //echo "<br><b style='color:red'>EMAIL NOT SENT</b><br> {$mail->ErrorInfo}", PHP_EOL; //Catch errors from Amazon SES.
    mysql_query("UPDATE `users` SET `unsubscribe` = '1' WHERE `emailaddress` = '$recipient' LIMIT 1");
    sendCloudwatchData('EtraGroupCrons', 'email-failure-abandoned', 'AbandonedCheckout', 'abandoned-checkout-failure-function', 1);
    writeCloudWatchLog('Crons-Emailer', $recipient .' Caught exception: '.  $mail->ErrorInfo);
}
