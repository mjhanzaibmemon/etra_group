    ...
    if ($data['event'] == 'fraud_new') {
        ...
        if (mysql_num_rows($checkOsQuery) > 0) {
            ...
            if (mysql_num_rows($checkExist) == 0) {
                ...
                writeCloudWatchLog('webhook-fraud-alert', "Inserted blacklist fraud new : {$dataOS['emailaddress']} or {$dataOS['igusername']} or {$dataOS['ipaddress']}");
            }
        }
    }
    ...
    if ($data['event'] == 'dispute_new') {
        ...
        if (mysql_num_rows($checkOsQuery) > 0) {
            ...
            if (mysql_num_rows($checkExist) == 0) {
                ...
                echo 'insert';
                sendCloudwatchData('EtraGroupWebhook', 'blacklist-insert', 'FraudAlert', 'blacklist-insert-function', 1);
                writeCloudWatchLog('webhook-fraud-alert', "Inserted blacklist dispute new : {$dataOS['emailaddress']} or {$dataOS['igusername']} or {$dataOS['ipaddress']}");
            }
        }
    }
