        ...
        if (!in_array($orderSession, $uniqueSessions)) {
            ...
            if (mysql_num_rows($checkOsQuery) > 0) {
                ...
                if (mysql_num_rows($checkExist) == 0) {
                    ...
                    writeCloudWatchLog('admin-fraud-import', "Inserted blacklist {$dataOS['emailaddress']} or {$dataOS['igusername']} or {$dataOS['ipaddress']}");
                    sendCloudwatchData('EtraGroupAdmin', 'blacklist-insert', 'FraudImport', 'blacklist-insert-function', 1);
