		...
		$endTime = date('Y-m-d H:i:s', strtotime("tomorrow", $orderDay) - 1); // currentdate end time
			...
			if (!empty($refundinfo['payment_id'])) {
					...
					if (empty($token_response['access_token'])) {
						$tpmsg = '<div style="padding:10px;">Authentication failed with payment gateway</div>';
					}
				
					$access_token = $token_response['access_token'];
					// echo 'Access Token: '.$access_token.'<hr>';die;
			...
			if(stripos($refundinfo['payment_id'], 'pi_') !== false || stripos($refundinfo['payment_id'], 'ch_') !== false){
				...
				try {
					...
					$refundData['amount'] = (int)(floatval($customamount) * 100);
...
if(!empty($alorderid) && empty($undoRefund)){
		...
		// Old Cardinity refund code (commented out)
		// try{
		// 	 $cardinityamt = floatval($customamount);
