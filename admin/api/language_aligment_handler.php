...
function finishEditing2(){
    ...
    if(empty($url) || empty($content)){
        echo json_encode(['message'=>'Content or URL is missing']);
        die;
    }
        
    $content_query = mysql_query("UPDATE `content` SET updated_by = '$adminName' WHERE `url` = '$url' AND `name` = '$name' limit 1");
    // Insert into history table
    $content_history = mysql_query("INSERT INTO `content_history` set content= '$content', `url` = '$url', `name` = '$name', updated_by = '$adminName', brand = '$brand',  `date` = '$now'");
...
function getHistoryContent2(){

    // page
    $url = addslashes($_POST['url']);
    $name = addslashes($_POST['name']);

    $content_history = mysql_query("SELECT * FROM `content_history` WHERE `url` = '$url' and `name` = '$name' order by `date` desc");
...
function approve(){
    ...
    $chid = addslashes($_POST['chid']);
    
    $content_history = mysql_query("SELECT * FROM `content_history` WHERE `id` = '$chid'");
    $content_history_data = mysql_fetch_array($content_history);

    $content = mysql_query("UPDATE `content` set content= '{$content_history_data['content']}' WHERE `id` = '$cid'");
    $content = mysql_query("UPDATE `content_history` set approved= '1' WHERE `id` = '$chid'");
    if($content){
...
function keepSame(){
    ...
    $cid = addslashes($_POST['cid']);
    $chid = addslashes($_POST['chid']);
    $content_history = mysql_query("DELETE FROM `content_history` WHERE `id` = '$chid'");
...
function deleteAll(){
    ...
    $content_history = mysql_query("DELETE FROM `content_history` WHERE `id` = '$chid'");
    // $content = mysql_query("DELETE FROM `content` WHERE `id` = '$cid'");
